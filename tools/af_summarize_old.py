...
def render_html(run_name, events, final_selection_rows, final_attacks_rows,
                sel_iters, att_iters, edge_reasons):
    css = """
    <style>
    ...
    # Per-iteration embedded tables
    html_parts.append("<h2>Per-iteration snapshots (embedded)</h2>")
    if sel_iters:
        # Align selection & attack files by iteration index
...
# -------------------------
def main():
    ap = argparse.ArgumentParser(description="Summarize AF logs into an HTML report with per-iteration tables.")
    ...
    html_out.write_text(
        render_html(run_name, events, final_selection_rows, final_attacks_rows, sel_iters, att_iters, edge_reasons),
        encoding="utf-8"
    )
